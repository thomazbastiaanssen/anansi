...
```
Note that anansi tells us how how the rewritten model looks. 

```{r plot_full, echo=FALSE, warning=FALSE, message=FALSE, fig.dim=c(6,3)}
library(patchwork)
library(ggplot2)
...
ggplot(full_df) +
  ...
  plot_annotation(
    subtitle = "Both citrate and cis-aconitate ~ aconitase dehydrogenase." 
    ) &
...
```{r disj_cat, echo=FALSE, fig.dim=c(6,3)} 
diff_df <- data.frame(
...
ggplot(diff_df) +
    ...
    labs(x = "isocitrate dehydrogenase", 
         subtitle = paste0("isocitrate ~ isocitrate dehydrogenase, ", 
...
```{r disj_cont, echo=FALSE, fig.dim=c(10,4)} 
ggplot(diff_df) +
    ...
    labs(x = "ketoglutatate dehydrogenase", 
         subtitle = paste0("ketoglutarate ~ ketoglutarate dehydrogenase ", 
                           "stratified by a third continuous variable.")
         ) +
    theme_bw() +
...
```{r emerg_cat, echo=FALSE, fig.dim=c(6,3)}
ggplot(diff_df) +
    ...
    labs(x = "succinyl-CoA synthetase", y = "succinyl-CoA", 
         subtitle = paste0("succinyl-CoA ~ succinyl-CoA synthetase ", 
                           "by category (treatment).")
         ) +
    theme_bw() +
    coord_equal(ratio = 1, xlim = c(-2.5, 2.5), ylim = c(-2.5, 2.5))
...
```{r emerg_cont, echo=FALSE, fig.dim=c(10,4)}
ggplot(diff_df) +
    ...
    labs(x = "succinate dehydrogenase", 
         subtitle = paste0("succinate ~ succinate dehydrogenase ", 
                           "stratified by a third continuous variable.")
         ) +
    theme_bw() +
    coord_equal(ratio = 1, xlim = c(-2.5, 2.5), ylim = c(-2.5, 2.5))
