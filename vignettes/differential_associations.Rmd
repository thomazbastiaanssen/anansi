...
positive or negative \@ref(fig:plot_full). 
...
the metabolite *flips*. \@ref(fig:disj_cat)
...
but rather the result of a continuous variable. \@ref(fig:disj_cont)
...
#| 
ggplot(diff_df) +
    aes(x = ketoglutarate.dehydrogenase, 
        y = ketoglutarate, 
        fill = pnorm(score_a)) +
    facet_grid(~score_a_cat) +
    geom_point(shape = 21, show.legend = FALSE) +
     scale_fill_gradientn(name = "Example health index",
...
go *out of sync*. \@ref(fig:emerg_cat)
...
under physiological conditions, but this association weakens along with some health index or environmental variable. \@ref(fig:emerg_cont)
