...
unique metabolites. Let's imagine we had (high-dimensional proxy) measurements 
...
data sets like two sets of features, as shown in figure \@ref(fig:plot-krebs)B.
...
krebs <- ggplot(kd) + 
    ...
    geom_point(aes(x = 0.75*cos(rad-pi/9), y =0.75* sin(rad-pi/9)), size = 3, 
               ...
               shape = 21) + 
    geom_text(aes(x = 1.15*cos(rad-pi/9), y =1.15* sin(rad-pi/9),
               label = enzyme), size = 2.4, show.legend = FALSE) + 
