...
```{r}
#| plot-krebs, echo=FALSE, fig.wide=TRUE, out.width='91.5%', fig.asp=3/5, 
...
#| and metabolites \\
#| involved in the pathway. In both figures, orange marks 
...
krebs <- ggplot(kd) + 
    ...
    geom_tile(aes(x = 0.8*cos(rad), y =0.8* sin(rad)), width = 0.45, 
              ...
              show.legend = FALSE) + 
    geom_text(aes(x = 0.8*cos(rad), y =0.8* sin(rad), label = metab), size = 3.5,
              show.legend = FALSE) + 
...
colmat <- data.frame(name = c(
  ...
  "Isocitrate\ndehydrogenase", "Malate\ndehydrogenase", "Succinyl-CoA\nsynthetase", 
  "α-Ketoglutarate\ndehydrogenase", "cis-Aconitate", "Citrate", "Fumarate", 
  "Isocitrate", "L-Malate", "Oxaloacetate", "Succinate", "Succinyl-CoA", 
  "α-Ketoglutarate"), 
...
p_0 <- ggplot(colmat) +
  ...
  geom_text(aes(label = name), colour = "black") + 
...
```{r}
#| all-v-all-p1, echo=FALSE, fig.wide=TRUE, out.width='91.5%', fig.asp=3/5, 
...
#| representation, with cells \\
#| marked with 'X' signifying tested feature-pairs."
...
```{r}
#| all-v-all-p2, echo=FALSE, fig.wide=TRUE, out.width='91.5%', fig.asp=3/5,
...
#| **B)** Matrix representation, with cells \\
#| marked with 'X' signifying tested feature-pairs."
