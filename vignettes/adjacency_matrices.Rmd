...
This vignette contains several sections:
...
work with it
* [Section 3](#sec-3) discusses additional methods to generate (bi)adjacency 
matrices. 
...
phosphorylation. See figure \@ref(fig:plot-krebs)A below for a simplified 
representation. In the figure, there are 8 unique enzymatic reactions and 9 
unique metabolites. 

```{r}
#| plot-krebs, echo=FALSE,
...
$8\times9=72$ hypotheses. Figure \@ref(fig:all-v-all-p1)A. 

```{r}
#| all-v-all-p1, echo=FALSE, fig.wide=TRUE, out.width='100%', out.height='60%',
...
\@ref(fig:all-v-all-p1)B. Non-selectively testing all 72 associations actively 
...
anansi requires a *biadjacency matrix*. The biadjacency matrix for the above 
figure can be seen in figure \@ref(fig:all-v-all-p2)B. 

```{r}
#| all-v-all-p2, echo=FALSE,
...
two `data.frames`, in fact. Let's take a look at them:
...
The `data.frame`s are structures similarly; Each column contains feature IDs of 
one type. The column names correspond to the types of feature IDs. If two IDs 
are adjacent, on the same row, it means the features are paired. 

## The pre-packaged kegg linking map
By default, `weaveWeb()` will assume to use the kegg link map, which consists of
both `data.frame`s in a list. We can call it directly using `kegg_link()`: 
```{r kegg_web}
...
Note the column names in the two dataframes: `ec`, `cpd` and `ko`. Two of these 
correspond to the formula we used just used: `weaveWeb(ec ~ ko)`. We didn't use 
the term `cpd`. Further, `ec` is present in both `data.frame`s. 

We can use weaveWeb to make a biadjacency matrix between any combination of one 
or two similarly structured `data.frame`s, presented as a list. In this example,
other words, we can link between any two of `ec`, `cpd` and `ko`, in either 
order. 

```{r web_formula}
# Formula notation
form.web <- weaveWeb(cpd ~ ko)

# Traditional
form.trad <- weaveWeb(y = "ko", x = "cpd")
```
...
# Defining an adjacency matrix {#sec-3}
