...
#'     ecs, typicallydescribe reactions captured by them.
#'
...
#' @rdname kegg_link
#' @format `ec2cpd`: a `data.frame` of two columns, named `"ec"` and `"cpd"`.
...
#' @description
#' `kegg_links` is a convenience function to return a list containing two
#' `data.frame`s; `ec2cpd` and `ec2ko`. This will be their most likely use.
#' `ec2cpd` and `ec2ko` are two `data.frame`s, used to link ko, ecs and cpd
...
#'
#' @description There were three treatment groups in the study that all received
#'     faecal microbiota transplantation (FMT). Young mice that received FMT
