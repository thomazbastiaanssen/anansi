...
anansiAdjustP <- function(x, method = "BH", resampling = F, locality = T, verbose = T) {
  ...
  if (!resampling & locality) {
    locality <- F
    if (verbose) {
      print("Locality is only available for resampling procedure. Setting locality to False")
    }
  }
  ...
  if (verbose) {
    ...
    if (resampling & locality) {
      print("Using locally resampled distributions. ")
    } else if (resampling & !locality) {
      print("Using total resampled distributions.")
    } else if (!resampling) {
      print("Using theoretical distribution.")
    }
  }
