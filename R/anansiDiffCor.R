...
anansiDiffCor = function(web, metadata, groups, formula, reff, modeltype, verbose = T){
  ...
  #return(list(web = web, stats_out = stats_out, all_terms = all_terms))
...
#' @param by.ls Boolean to toggle ls calculation method.
#' @return a list of \code{anansiTale} result objects, one for the total model, one for emergent correlations and one for disjointed correlations.
#'
collate_model_output <- function(web, stats_out, all_terms, by.ls = FALSE){
...
#' @return a list of \code{anansiTale} result objects, one for the total model, one for emergent correlations and one for disjointed correlations.
#'
collate_model_output_ls <- function(web, stats_out, all_terms){
  ...
  n <- 3  #   (R, F, p)
  p <- 2  # * emerg + disj
  trms <- length(all_terms)

  t <- 1:trms
  r_disj.ind <- 1 + n + (t-1) * n*p
  r_emer.ind <- r_disj.ind + 1
  p_disj.ind <- r_disj.ind + 2*p
  p_emer.ind <- p_disj.ind + 1
  ...
  out_pvals[web@dictionary]      <- stats_out[,n]
  ...
  out_disjointed = vector(mode =  "list", length = trms)

  for(t in 1:trms){
    out_disjrvals[web@dictionary]  <- stats_out[,r_disj.ind[t]]
    out_disjpvals[web@dictionary]  <- stats_out[,p_disj.ind[t]]
  ...
  out_emergent = vector(mode =  "list", length = trms)

  for(t in 1:trms){
    out_emergrvals[web@dictionary]  <- stats_out[,r_emer.ind[t]]
    out_emergpvals[web@dictionary]  <- stats_out[,p_emer.ind[t]]
