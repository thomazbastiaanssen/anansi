...
- [(mia contributor guidelines)](https://github.com/microbiome/mia/blob/devel/CONTRIBUTING.md)
...
machine using git.
Here's an example of cloning, assuming you already forked `anansi`:
...
see the contact details at [thomazbastiaanssen.github.io](thomazbastiaanssen.github.io).
...
mention one of the maintainers of the repo in a comment e.g. `@thomazbastiaanssen`
...
1. _Fork_ (with Git) anansi R package from [https://github.com/thomazbastiaanssen/anansi]

1. _Clone_ (with Git) your own fork of anansi; or if you have cloned it
...
After accepted pull request, check if further updates are needed in other 
related packages (e.g. MultiFactor, ariadne, pallas)
...
This is usually done only by the main developer. In this case, the upstream will be set to Bioconductor: `git remote add upstream git@git.bioconductor.org:packages/anansi.git`
...
- [Package page in Bioconductor](https://bioconductor.org/packages/anansi/)
